# Handyman App – Architecture Overview

## Overview
Handyman App is a closed‑source Windows `.exe` application built for offline operation, secure license enforcement, and automated reporting. It provides a fast, local Operational CRM tailored specifically for handyman and general repair workflows. Unlike cloud‑based CRMs, the system runs entirely on the user’s machine or internal network, ensuring reliability, data ownership, and zero external dependencies.
...
- Admin enters job details, worker info, materials used, and tasks completed
- Payroll supports multiple calculation methods:
  - Task count × rate (e.g., repairs, installations, assemblies)
  - Hourly wage
  - Piece rate for specific tasks
  - Flat rate per job
