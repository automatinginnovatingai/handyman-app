...
• Payroll Management:
  ...
  - Calculate handyman task and material usage using:
      • Quantity × Pay Rate (e.g., 3 door repairs × $45.00/task)
      • Per-piece rates (e.g., 12 anchors × $0.50)
      • Hourly wage (e.g., 8 hours × $25.00)
      • Enter 0 in any quantity or rate field that does not apply
  - Add up to four workers for each job.
  - Automatic pay splitting among listed workers.

• Worker Job Entry:
  Each job entry will require:
      - First and last name of each worker
      - Worker ID (if applicable)
      - Work hours and job roles
      - Client, property address, unit/room details
      - Optional: Handyman task and material info such as:
        • Hardware installed (e.g., brackets, anchors, hinges)
        • Repairs performed (e.g., drywall patch, door adjustment)
        • Installations (e.g., shelving, fixtures, appliances)
        • Materials used (e.g., caulk, screws, paint, lumber)
...
• Pay Week Entry & Auto Export
  ...
  • What Gets Exported
  - Weekly worker payroll grouped by name or ID
...
Getting Started
---------------
1. Launch the app via the Handyman App icon.
...
5. Click 'Worker Worksheet' to begin logging job and payroll data.
...
Thank you for using Handyman App!